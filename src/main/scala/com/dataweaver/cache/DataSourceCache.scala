...
import com.dataweaver.factories.DataSourceFactory
...
object DataSourceCache {
   ...
   *
   * @param sourceConfig The configuration of the data source.
   * @param spark        The SparkSession to use for DataFrame operations.
   * @return A DataFrame for the data source.
   */
  def getOrCreate(sourceConfig: DataSourceConfig)(implicit spark: SparkSession): DataFrame = {
    ...
    cache.getOrElseUpdate(sourceConfig.id, {
