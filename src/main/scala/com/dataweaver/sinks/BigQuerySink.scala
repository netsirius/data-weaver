 ...
 */
class BigQuerySink(config: SinkConfig, writeToFile: Boolean = false, outputFile: Option[String] = None) extends DataSink {
  ...
  override def writeData(data: DataFrame, pipelineName: String)(implicit spark: SparkSession): Unit = {
    ...
    val saveMode = config.config.getOrElse("saveMode", throw new IllegalArgumentException("GCS Bucket is required"))
