...
class DataFlowExecutor(appConfig: DataWeaverConfig) {
  ...
  /** Executes data flows based on the specified tag and regex criteria.
    *
    * @param tag
    *   Optional tag for filtering data flows.
    * @param regex
    *   Optional regular expression for filtering YAML files.
    */
  def executeDataFlows(
      tag: Option[String],
      regex: Option[String],
  ...
  /** Retrieves the YAML files to be processed, filtered by the provided tag and regex.
    ...
    * @param regex
    *   Optional regex for filtering.
    * @return
    *   A sequence of Paths to the YAML files.
    */
  ...
  private def runPipeline(
      pipelineConfig: DataPipelineConfig
  )(implicit spark: SparkSession): Unit = {
    try {
  ...
  private def updateConfigTypes(configs: Seq[DataPipelineConfig]): Seq[DataPipelineConfig] = {
    configs.map { pipelineConfig =>
      val updatedDataSources =
        pipelineConfig.dataSources.map(dataSource => dataSource.copy(`type` = "Test"))
  ...
  /** Transforms the types of data sources and sinks based on the execution mode.
    ...
    * @return
    */
  def transformConfigs(
      configs: Seq[DataPipelineConfig],
      mode: ExecutionMode
  ): Seq[DataPipelineConfig] = {
    mode match {
